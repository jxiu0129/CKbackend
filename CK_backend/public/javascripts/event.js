...
const findUser = () => {
    ...
    // location.href = 'attend.nccu.edu.tw/eventslist?search=' + target;
    $.ajax({
        url: `https://attend.nccu.edu.tw/sponsor/events/:eventid/attendancelist?search=${target}`,
        success: () => {
            console.log('success find');
            window.location.href = `https://attend.nccu.edu.tw/sponsor/events/:eventid/attendancelist?search=${target}`;
        },
        error: () => console.log('fail to find')
    });
    console.log(target);
...
const downloadCSV = (id) => {
    ...
    if(btn){ 
        $.ajax({
            url: `https://attend.nccu.edu.tw/exportCSV/${id}`,
            success: () => {
